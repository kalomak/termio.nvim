-- Simple terminal read/write API with a bundled popup editor.
...
---@param tbufcache TerminalBufferCache
---@return { lines: string[], cursor: integer[], first_row: integer }
...
M.disable = function()
  ...
  vim.api.nvim_del_augroup_by_name("termio-standalone-popup")
  for buf in pairs(M.terminals) do
    vim.keymap.del("t", M.options.key, { buffer = buf })
  end
  M.enabled = false
  return M
end
...
M.float_config = function(tbufcache)
  ...
  local command_pos = vim.fn.screenpos(
    tbufcache.target_win,
    tbufcache.command_start[1],
    tbufcache.command_start[2] + 1
  )
  local window_pos = vim.fn.win_screenpos(tbufcache.target_win)
  return {
    relative = "win",
    ...
    width = width,
    height = height,
    row = command_pos.row - window_pos[1],
    col = 0,
    style = "minimal",
    border = "none",
  }
